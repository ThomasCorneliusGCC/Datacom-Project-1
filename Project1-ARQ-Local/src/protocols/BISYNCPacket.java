...
public class BISYNCPacket {
    ...
    private byte[] byteStuff(byte[] data) {
        int stuffedSize = data.length;

        for(int i=0; i<data.length; i++){
            if(data[i] == STX || data[i] == SYN || data[i] == DLE || data[i] == ETX){
                stuffedSize ++;
            }
        }
        byte[] stuffed = new byte[stuffedSize];
        int index = 0;
        for(int i = 0; i<data.length; i++){

            if(data[i] == STX || data[i] == SYN || data[i] == DLE || data[i] == ETX){
                stuffed[index] = DLE;
                stuffed[index + 1] = data[i];
                index += 2;
            } else {
    ...
    private byte[] byteUnstuff(byte[] stuffedData) {
        ...
        int dataSize = stuffedData.length;
        ...
        byte[] unstuffed = new byte[dataSize];
